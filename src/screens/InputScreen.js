...
import { View, Text, StyleSheet, Image, ScrollView, TextInput, TouchableOpacity, Button } from 'react-native';
            ...
            <View style={styles.inputContainer}>
                {inputData.inputs.map((input, index) => {
                  return (
                      <View 
                        key={index}
                          style={[styles.inputView, { width: INPUT_NUMBER_WIDTH_MAP[inputData.inputs.length]}]}>
                        {input.label ? <Text style={styles.label}>
                            {input.label}
                        </Text> :  null}
                        <TextInput
                            ...
                            style={[styles.inputContent, input.multiline ? {minHeight: 180} : {}]}
